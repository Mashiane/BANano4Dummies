...
Sub createElement(tag As Object) As BANanoElement
	Dim newEL As BANanoObject = JQuery.Selector(tag)
	Return BANano.ToElement(newEL)
End Sub
...
Sub MoveDown(itm As BANanoElement)
	Dim sNext As BANanoElement = BANano.ToElement(itm.GetField("nextSibling"))
	Dim s As BANanoObject = BANano.ToObject(itm)
	Dim t As BANanoObject = BANano.ToObject(sNext)
	JQuery.Selector(s).RunMethod("insertAfter", t)
End Sub
...
Sub AppendMulti(elID As BANanoElement, toAppend As List) As BANanoJQuery
	Dim p As BANanoObject = BANano.ToObject(elID)
	For Each obj As BANanoElement In toAppend
		Dim child As BANanoObject = BANano.ToObject(obj)
		p.RunMethod("append", Array(child))
	Next
	Return Me
End Sub
